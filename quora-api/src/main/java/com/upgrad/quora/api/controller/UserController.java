...
public class UserController {
  // Required services are autowired to enable access to methods defined in respective Business
  // services
  @Autowired private SignupBusinessService signupBusinessService;

  @Autowired private SigninBusinessService signinBusinessService;

  @Autowired private SignoutBusinessService signoutBusinessService;

  /**
   * @param signupUserRequest the first {@code SignupUserRequest} to signup a particular user with
   *     details.
  ...
  public ResponseEntity<SigninResponse> login(
    ...
    UserAuthTokenEntity userAuthToken =
        signinBusinessService.authenticate(authArray[0], authArray[1]);
  ...
  @RequestMapping(
      ...
      path = "/user/signout",
      produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
  public ResponseEntity<SignoutResponse> loginOut(
    ...
    if (userAuthToken != null) {
      signoutResponse =
          new SignoutResponse().id(userAuthToken.getUuid()).message("SIGN OUT SUCCESSFULLY");
    }
